	...
	private void OnGUI()
	{
		...
		if (isOpen)
		{
			...
			if (command != text4)
			{
				...
				if (textEditor != null)
				{
					commandLastPos = textEditor.selectIndex;
					commandLastSelectPos = textEditor.selectIndex;
				}
			}
			GUILayout.EndHorizontal();
			GUILayout.EndArea();
			if (Event.current.type == EventType.Repaint && moveCursorToEnd)
			{
				...
				if (textEditor2 != null)
				{
					...
					textEditor2.selectIndex = textEditor2.selectIndex;
					textEditor2.graphicalCursorPos = textEditor2.style.GetCursorPixelPosition(new Rect(0f, 0f, textEditor2.position.width, textEditor2.position.height), textEditor2.content, textEditor2.selectIndex);
					commandLastPos = textEditor2.selectIndex;
					commandLastSelectPos = textEditor2.selectIndex;
				}
		...
		if (isOpen && Event.current.type == EventType.Layout && GUI.GetNameOfFocusedControl() != "CommandTextField")
		{
			...
			if (textEditor3 != null)
			{
				textEditor3.selectIndex = commandLastPos;
				textEditor3.selectIndex = commandLastSelectPos;
			}
		}
	}
