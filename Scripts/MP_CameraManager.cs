...
public class MP_CameraManager : MonoBehaviour
{
	public static MP_CameraManager instance;

	public int currentCameraID;

	public MP_CameraComputer cameraComp;
	...
	private void Update()
	{
		for (int i = 0; i < gameCameras.Count; i++)
		{
			if ((bool)gameCameras[i])
			{
				cameraButtons[i].interactable = gameCameras[i].gameObject.activeSelf;
			}
			else
			{
	...
	[RPC]
	public void AddCamera(Transform id)
	{
		gameCameras.Add(id);
		RefreshButtons();
	}
	...
	public void ActiveCamera(int id)
	{
		for (int i = 0; i < gameCameras.Count; i++)
		{
			if (i == id && gameCameras[i] != null)
			{
				...
				gameCameras[i].parent.GetComponent<MP_RecordCamera_obj>().isRecord = true;
